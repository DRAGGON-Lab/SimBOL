...
# =============================================================================
# CIRCUIT ANALYSIS
# Resolve the full regulatory chain before generating any code.
# =============================================================================
...
def analyse_circuit(proteins, interactions, component_map):
    ...
    The key insight: inhibitions in SBOL target PROMOTERS, not proteins directly.
    We resolve this by tracing promoter → proteins produced from it.
...
def generate_update_logic(proteins, modules, interactions, component_map, params,
    ...
    Inhibitions on promoters are correctly traced through to the proteins
    they control — no promoter IDs ever appear as cell state variables.
...
# =============================================================================
# COLOUR HELPER
# Generate a GFP-level or protein-level colour expression for visualisation.
# =============================================================================
...
def generate_script(proteins, modules, interactions, component_map, params,
    ...
    if ed_chemicals:
        ...
        chem_consts_section = (
            ...
            "# They are NOT cell-state variables; adjust the constants below to simulate\n"
    ...
    # --- Summary comment ---
    module_summary  = ", ".join(
        f"{m['name']} ({'constitutive' if m['constitutive'] else 'regulated'})"
        for m in modules
    ) or "none"
    protein_summary = ", ".join(p["display_id"] for p in proteins) or "none"
    chem_summary    = ", ".join(
        f"{c['display_id']} ({c['type']})" for c in (ed_chemicals or [])
    ) or "none"
    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M")

    script = f'''# =============================================================================
# CellModeller Script — Auto-generated from SBOL JSON
# Generated: {timestamp}
# =============================================================================
# Modules:   {module_summary}
# Proteins:  {protein_summary}
# Chemicals: {chem_summary}
# =============================================================================
