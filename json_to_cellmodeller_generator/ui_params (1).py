...
def display_form(sbol_data, ignore_component_ids=None):
    ...
    grid_len_w = widgets.IntText(
        value=DEFAULT_SIGNALING["grid_len"], description="Grid cells per axis:",
        style={"description_width": "initial"},
    )
    ...
    boundary_w = widgets.Dropdown(
        options=["fixed", "periodic"], value=DEFAULT_SIGNALING["boundary_condition"],
    ...
    if diffusible_signals:
        ...
        for sid in sorted(diffusible_signals):
            ...
            deg_w = widgets.FloatText(
                value=DEFAULT_SIGNAL_RATES["degradation_rate"], description="Degradation rate:",
                style={"description_width": "initial"},
            )
            row = widgets.VBox(
                [info_html, widgets.HBox([diff_w, _spacer(), deg_w])],
                layout=widgets.Layout(border="1px dashed lightgray", margin="5px 0", padding="8px"),
            )
            signal_rows.append(row)
            signal_rate_widgets[sid] = {"diffusion": diff_w, "degradation": deg_w}
    ...
    def _add_custom_signal(_btn=None):
        ...
        row = widgets.HBox([name_w, _spacer(), diff_w, _spacer(), deg_w, _spacer(), remove_btn])
        box = widgets.VBox([row], layout=widgets.Layout(border="1px dashed lightgray", margin="5px 0", padding="8px"))
        entry = {"box": box, "widgets": {"name": name_w, "diffusion": diff_w, "degradation": deg_w}}
    ...
    signaling_box = _section(
        ...
        widgets.HBox([grid_len_w, _spacer(), grid_size_w, _spacer(), boundary_w]),
    ...
    def _collect_parameters():
        ...
        for sid, rw in signal_rate_widgets.items():
            signals.append({
                ...
                "degradation_rate": rw["degradation"].value,
            })
        for entry in custom_signal_entries:
            ...
            if w["name"].value.strip():
                signals.append({
                    ...
                    "degradation_rate": w["degradation"].value,
                })
