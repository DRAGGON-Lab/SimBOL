...
DEFAULT_SIGNAL_RATES = {
    "diffusion_rate":         0.1,
    # NOTE: GridDiffusion has no field-wide degradation parameter in the
    # real CellModeller API — signal only leaves the grid via cell-mediated
    # exchange (sigRateCL). This rate controls how fast a cell
    # exchanges the signal with the grid at its location; it defaults to
    # diffusion_rate in generate_script() if omitted.
...
DEFAULT_SIGNALING = {
    "enabled":            True,
    "grid_len":           100,
    # Number of grid cells along z. Keep this small (1-4) for a 2D/monolayer
    # simulation. GridDiffusion requires grid_size to be the SAME along x, y
    # and z, so a thin domain is made by shrinking the cell *count* in z,
    # not the cell size.
    "grid_z_cells":       1,
    "grid_size":          4.0,
    ...
    # given explicitly. Set parameters["signaling"]["grid_origin"] = [x,y,z]
    # to override.
...
def detect_signals(sbol_data, ignore_component_ids=None):
    ...
    to know up front which signals need diffusion/membrane-exchange rate
...
def _deep_merge_parameters(parameters, overrides):
    ...
    if "signaling" in overrides:
        ...
        for key in ("enabled", "grid_len", "grid_z_cells", "grid_size",
                    "grid_origin", "boundary_condition"):
            if key in sig_over:
