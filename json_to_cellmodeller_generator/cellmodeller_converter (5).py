...
def generate_script(proteins, modules, interactions, component_map, params,
    ...
    # grid_z_cells  = number of grid cells along z. MUST be >= 2 — GridDiffusion's
    #                 trilinear interpolation (interpAddToGrid/trilinearWeights)
    #                 unconditionally reads grid index (pidx + 1) along every
    #                 axis, even for corners whose weight is zero, so an axis
    #                 with only 1 cell always raises IndexError. Use a small
    #                 value like 2-4 for a thin/2D-ish simulation, never 1.
